...
} from "./infiniteCanvas/TransformContext";
...
export function createDraggable<T extends DraggableItem>(
  ...
  /**
   * Update an item's position in the store using produce for efficient updates
   */
  ...
  const handleDragStart = (e: MouseEvent, id: string): void => {
    // CRITICAL FIX: Cancel any existing drag first
    cancelDrag();
    ...
    const moveHandler = (moveEvent: MouseEvent): void => {
      ...
      const currentWorldPos = transformContext
        ? transformContext.screenToWorld({
            x: currentScreenX,
            y: currentScreenY,
          })
        : {
      ...
      // Batch the store update and callback for smooth performance
      batch(() => {
  ...
  const handleDragMove = (id: string, delta: Vector): void => {
    ...
    // Batch all updates for smooth performance
    batch(() => {
      ...
      setDragState({
        ...
        currentScreenX:
          currentState.startScreenX +
          (transformContext
            ? transformContext.scaleWorldDistance(delta.dx)
            : delta.dx),
        currentScreenY:
          currentState.startScreenY +
          (transformContext
  ...
  const bringToFront = (id: string): void => {
    ...
    if (canvasAPI && canvasAPI.setItemZIndex) {
      ...
      const range = id.startsWith("menu-")
    ...
    if (draggedId() === id) {
      ...
      if (index !== -1) {
        setItems(index, produce((item) => {
          item.zIndex = Z_INDEX_RANGES.DRAGGING.MAX;
        }));
      }
    }
  };
