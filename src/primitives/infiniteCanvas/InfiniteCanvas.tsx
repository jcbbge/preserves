...
export function InfiniteCanvas(props: InfiniteCanvasProps) {
  ...
  const loadViewport = () => {
    ...
    try {
      ...
      if (stored) {
        ...
        // Check if parsed viewport is valid
      ...
      // Silent fail - viewport will use defaults
    }
  };
  ...
  const updatePan = (clientX: number, clientY: number) => {
    ...
    if (interactionManager.state.type !== "pan" || !interactionManager.state.active) {
      return;
    }
    ...
    const delta = interactionManager.getDelta();
    if (!delta) return;
    
    // Ensure we have starting viewport information
    if (!interactionManager.state.startViewport) return;
  ...
  const handleTouchMove = (e: TouchEvent) => {
    ...
    if (interactionManager.state.type === "pan" && 
  ...
  const zoomAtPoint = (pointX: number, pointY: number, scaleFactor: number) => {
    ...
    if (bounds) {
      finalPositionX = Math.max(bounds.minX, Math.min(bounds.maxX, finalPositionX));
      finalPositionY = Math.max(bounds.minY, Math.min(bounds.maxY, finalPositionY));
    }
  ...
  const zoomTo = (scale: number, center?: Point, options?: { animate?: boolean }) => {
  ...
  const isItemVisible = (itemId: string): boolean => {
    ...
    if (props.onGetItemPosition) {
      ...
      if (position) {
        ...
        return transform.isWorldPointVisible(position);
      }
    }
  ...
  const isRectVisible = (rect: { x: number; y: number; width: number; height: number }): boolean => {
    const container = containerRef();
  ...
  // Z-index management
  const [zIndexRegistry, setZIndexRegistry] = createStore<{ id: string; zIndex: number; itemType: 'photo' | 'menu' | 'system' }[]>([]);
  ...
  const setItemZIndex = (id: string, position: "front" | "back" | number) => {
    ...
    const itemType = id.startsWith('menu-') ? 'menu' : 
                    id.startsWith('system-') ? 'system' : 'photo';
    
    // Determine which range to use
    const range = itemType === 'menu' ? Z_INDEX_RANGES.MENU_ITEMS :
                 itemType === 'system' ? Z_INDEX_RANGES.SYSTEM :
    ...
    if (registryIndex >= 0) {
      ...
      setZIndexRegistry(registry => [...registry, { 
        id, 
        zIndex: newZIndex,
        itemType
      }]);
      ...
      <TransformProvider viewport={viewport} containerRect={containerRect()}>
        <div
          ...
          class={`${styles["infinite-canvas-container"]} ${
            interactionManager.state.type === "pan" && interactionManager.state.active 
              ? styles.grabbing 
              : ""
          } ${props.className || ""}`}
          style={{
