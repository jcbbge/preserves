...
import { debugLog, DEV_MODE, USE_MOCK_DATA_FALLBACK } from './utils';
...
export async function fetchPosts(
  ...
  debugLog('posts', 'Fetching posts from Peach API', { hasToken: !!token, username });
  
  try {
    // Use provided username or extract from token
    const userIdentifier = username || extractUsername(token);
    if (!userIdentifier) {
      throw new Error('Unable to determine username. Please provide a username in the options.');
    }
 ...
 */
function generateMockPosts(username: string, count: number): PeachPost[] {
  debugLog('mock', `Generating ${count} mock posts for ${username}`);
  
  const posts: PeachPost[] = [];
  
  for (let i = 0; i < count; i++) {
    const post: PeachPost = {
      id: `mock-post-${i}-${Date.now()}`,
      createdTime: new Date(Date.now() - i * 3600000).toISOString(),
      message: [
        {
          type: 'text',
          text: `This is mock post #${i + 1} for testing purposes.`
        }
      ],
    ...
    // Add some dummy media to some posts
    if (i % 3 === 0) {
      post.media = [
        {
          type: 'image',
