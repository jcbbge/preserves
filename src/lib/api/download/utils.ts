...
export async function validateMediaFormat(blob: Blob, contentType: string): Promise<MediaValidationResult> {
  if (!blob || blob.size === 0) {
    return { 
      valid: false, 
      type: 'unknown',
  ...
  else if (array[0] === 0x89 && array[1] === 0x50 && 
      array[2] === 0x4E && array[3] === 0x47 &&
      array[4] === 0x0D && array[5] === 0x0A &&
...
export async function writeDebugFile(data: ArrayBuffer | Blob, filename: string): Promise<void> {
  if (!DEV_MODE) return;
  ...
  try {
    ...
    debugLog('debug', `Wrote debug file: ${debugPath} (${buffer.length} bytes)`);
  } catch (error) {
