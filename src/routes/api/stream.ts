...
export const mediaProxy = action(async (formData: FormData) => {
  ...
  try {
    ...
    if (!response.ok) {
      console.error(
        "[SERVER-PROXY] Media fetch error:",
        response.status,
        response.statusText,
      );
    ...
    if (
      ...
      return new Response("Received HTML instead of media content", {
        status: 422,
      });
    }
...
export const fetchStream = query(async (formData: FormData) => {
  ...
  try {
    ...
    try {
      ...
      console.log(
        "[SERVER-API] Wrote raw response to /tmp/peach_api_response.json",
      );
    } catch (writeError) {
      console.error(
        "[SERVER-API] Could not write response to file:",
        writeError,
      );
    }
